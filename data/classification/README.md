...
- 公开 OJ 题面补采：88 条 `source_extracted` 题面、88 组样例，当前图片总数 2，86 条标记为 `none_found`。
- AI C++ 参考解：88 条 `ai_generated_sample_verified`，均通过当前公开样例，答案状态仍为 `needs_review`。
- 补充题分类修正：88 条，依据公开题面和样例通过的 AI C++ 参考解，标签仍为 `needs_review`；其中 28 道一级题、28 道二级题和 10 道三级题已归入“基础程序设计 / 数论 / 排序模拟 / 贪心”等目录。
